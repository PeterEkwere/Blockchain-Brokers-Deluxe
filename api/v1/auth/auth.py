...
class Auth:
    ...
    def check_expired_positions(self, id):
        """ This method check and updates the users trade positions
        ...
        if user.switch_check == 'demo':
            open_positions = user.demo_open_positions  # Assuming open_positions is a list of position dictionaries
        else:
            open_positions = user.live_open_positions


        current_time = datetime.datetime.now(datetime.timezone.utc) 
        for position in open_positions:
            expiration_date = datetime.strptime(position['expiration_date'], '%Y-%m-%d %H:%M:%S')
            if current_time >= expiration_date:
                expired_positions.append(position)
                
                if user.switch_check == 'demo':
                    # Here we want to add the expired position to the users closed positions
                    user.demo_closed_positions[position['position_id']] = position
                    self._db.save()
                    ...
                    user.demo_earnings[position['position_id']] = earnings_data
                    self._db.save()
                    
                    # Here we want to remove that position from the users open positions
                    updated_open_positions = [pos for pos in open_positions if pos['position_id'] != position['position_id']]
                    self._db.update_user(user.id,  position=updated_open_positions)
                else:
                    # here we want 
                    pass

                # Remove expired position from open_positions
                #open_positions.remove(position)

        # Update user's open_positions in the database
        #user.open_positions = open_positions
        #save_changes(user)
